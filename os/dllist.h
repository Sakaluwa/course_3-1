...
public:
	...
	void* Remove(int* keyPtr);//remove from head of list,set *keyPtr to key of the remove item,return item(or NULL if list is empty)
	
	bool IsEmpty();//return true if list has elements
	//routines to put/get items on/off list in order (sorted by key)
	void SortedInsert(void* item, int sortKey);
	void *SortedRemove(int sortKey);//remove first item with key==sortKey,return NULL if no such item exists
